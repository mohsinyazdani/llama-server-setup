...
**Примечание**: В зависимости от настройки вашей системы, вам может потребоваться использовать:
- `python3 main.py` (Linux/macOS, или если Python 3 установлен как python3)
- `py main.py` (Windows, если используется Python launcher)
- `python main.py` (Windows, если Python в PATH)

3) Запустите сервер с помощью сгенерированного скрипта:
   - **Windows (PowerShell)**: `.\start.bat`
   - **Windows (Командная строка)**: `start.bat`
   - **Linux/macOS**: `./start.sh`

**Примечание**: В PowerShell необходимо использовать префикс `.\` перед файлами `.bat` из-за политики выполнения. В Командной строке вы можете запускать `start.bat` напрямую.
